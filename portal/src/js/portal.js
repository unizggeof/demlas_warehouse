(function (win, doc) {
	...
	function getCategory() {
		...
		if (searchConcept === "Pokrov") {
			category += "&_cat=demlas_lc";
			searchText += "<li>Tema podataka: Pokrov</li>";
		}
		if (searchConcept === "Katastar") {
			category += "&_cat=demlas_cp";
			searchText += "<li>Tema podataka: Katastar</li>";
		}
		if (searchConcept === "Ortofotografije") {
			category += "&_cat=demlas_oi";
			searchText += "<li>Tema podataka: Ortofotografije</li>";
		}
		if (searchConcept === "Mjerenja") {
			category += "&_cat=demlas_om";
			searchText += "<li>Tema podataka: Mjerenja</li>";
		}
		if (searchConcept === "Elevacija – Pokrivenost Mrežom") {
			category += "&_cat=demlas_el_cov";
			searchText += "<li>Tema podataka: Elevacija – Pokrivenost Mrežom</li>";
		}
		if (searchConcept === "Elevacija – TIN") {
			category += "&_cat=demlas_el_tin";
			searchText += "<li>Tema podataka: Elevacija – TIN</li>";
		}
		if (searchConcept === "Elevacija – Vektorski Element") {
			category += "&_cat=demlas_el_vec";
			searchText += "<li>Tema podataka: Elevacija – Vektorski Element</li>";
		}
		if (searchConcept === "Postojeća Uporaba Zemlje") {
			category += "&_cat=demlas_elu";
			searchText += "<li>Tema podataka: Elevacija – Postojeća Uporaba Zemlje</li>";
		}
		if (searchConcept === "Sve") {
			category += "";
			searchText += "<li>Sve podaci</li>";
		}
	}
	...
	var dof = new ol.layer.Tile({
			...
			title : "Ortofoto - DOF5 (DGU geoportal)",
			visible : false,
			opacity : 0.75
		});
	...
	var dof_toponimi = new ol.layer.Tile({
			...
			title : "Ortofoto s toponimima - DOF5 (DGU geoportal)",
			visible : false,
			opacity : 0.75
		});
	...
	var hok = new ol.layer.Tile({
			...
			title : "Hrvatska osnovna karta 1:5000 (DGU geoportal)",
			visible : false,
			opacity : 0.75
		});
	...
	var tk25 = new ol.layer.Tile({
			...
			title : "Topografska karta 1:25000 (DGU geoportal)",
			visible : false,
			opacity : 0.75
		});
	var map = new ol.Map({
			layers : [
				new ol.layer.Group({
					title : 'Base Maps',
					layers : [dof, dof_toponimi, hok, tk25, osmLayer, hvar2017]
				})/*,
	...
	// CENTER POSITION PLUS ZOOM IN URL HASH
	...
	$("#languages").on('click', (function openLocation() {
			var view = map.getView();
			var center = ol.proj.transform(view.getCenter(), 'EPSG:3857', 'EPSG:4326');
			window.location = '/warehouse/portal/en/#c=' + center[0] + ':' + center[1] + ':' + view.getZoom();
		}));
	...
	map.on('singleclick', function (evt) {
		...
		if (poi_enabled) {
			...
			$("#md-info-body").html("<p>Results overlapping point: <br>" + HDMS + "<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
	...
	$('#btn-search').click(function (e) {
		...
		if (bbox_enabled && searchTerm === '') {
			...
			console.log(url);
			$('#md-info').modal('show');
			$("#md-info-body").html("<p>Results overlapping BBOX:<br> " + bounds[0] + ", " + bounds[1] + ", " + bounds[2] + ", " + bounds[3] + "<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
		}
		if (bbox_enabled && searchTerm != '') {
			var bounds = ol.proj.transformExtent(map.getView().calculateExtent(map.getSize()), 'EPSG:3857', 'EPSG:4326');
			console.log(bounds);
			...
			console.log(url);
			$('#md-info').modal('show');
			$("#md-info-body").html("<p>Geospatial search applied with bounds: " + bounds[0] + ", " + bounds[1] + ", " + bounds[2] + ", " + bounds[3] + "<br>Search term: " + searchTerm + "<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
		}
		if (searchTerm != '' && (!bbox_enabled && !geoloc_enabled)) {
			...
			$('#md-info').modal('show');
			$("#md-info-body").html("<p>Results for: '" + searchTerm + "'<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
			//doSearch(searchTerm);
			$("#srch-term").val('');
		...
		if (searchTerm === '' && (!bbox_enabled && !geoloc_enabled)) {
			...
			$('#md-info').modal('show');
			$("#md-info-body").html("<p>Entire catalogue search applied: <br>"+ searchText +"<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
			//doSearch(searchTerm);
			$("#srch-term").val('');
		}
		...
		if (geoloc_enabled && searchTerm === '') {
			...
			$('#md-info').modal('show');
			//$('.modal-title').append("<p>Results covering location with Latitude:" + LAT +" and Longiture" + LON +"</p>");
			$("#md-info-body").html("<p>Results covering current geographic location:<br><b>Longitude: " + x + "<br>Latitude: " + y + "<br><iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
		}
	...
	$('#srch-term').keypress(function (e) {
		...
		if (e.which == '13') {
			...
			if (searchTerm === '' && (!bbox_enabled && !geoloc_enabled)) {
				var url = gnk + '?any=' + category;
				$('#md-info').modal('show');
				$("#md-info-body").html("<p>Entire catalogue search applied<iframe src='" + url + "' width='100%' height='600px' ></iframe></p>");
				//doSearch(searchTerm);
				$("#srch-term").val('');
			}
	...
	$('[id^="geoinput-"]').change(function () {
		if ($(this).parent().hasClass('toggle btn btn-mini btn-primary')) {
			...
			$("#spatialQueryType").find('span').text(typeEnabled + ' opcija omogućena');
		} else {
...
function addToMap(){
	alert("SUCCESS: FUNCTION WAS CALLED FROM GNK TEMPLATE!");
